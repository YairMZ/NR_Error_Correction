"""entropy based flipping decoder running script
"""
import pickle
...
from decoders import EntropyBitwiseDecoderSingleStructureFlipping
...
parser.add_argument("--ent_threshold", default=0.47, help="entropy threshold to be used in entropy decoder", type=float)
...
for p in bit_flip_p:
    ldpc_decoder = DecoderWiFi(spec=WiFiSpecCode.N1944_R23, max_iter=ldpc_iterations, channel_model=bsc_llr(p=p))
    entropy_decoder = EntropyBitwiseDecoderSingleStructureFlipping(
        DecoderWiFi(spec=WiFiSpecCode.N1944_R23, max_iter=ldpc_iterations, channel_model=bsc_llr(p=p)),
        seg_iter, model_length, thr)
    ...
    for tx_idx in range(n):
        ...
        d = entropy_decoder.decode_buffer(corrupted)
